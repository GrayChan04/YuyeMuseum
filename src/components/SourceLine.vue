...
const isUnavailable = computed(() => props.source.status === 'unavailable')
...
const archiveText = computed(() => {
  ...
  return props.source.archiveUrl ? '查看网页存档' : '查看馆内存档'
})
...
<template>
  <div class="source-line" :class="{ 'source-line--unavailable': isUnavailable }">
    <div class="source-line__reference">
      ...
      <span v-if="isUnavailable" class="source-line__status">原链接已失效</span>
      <a
